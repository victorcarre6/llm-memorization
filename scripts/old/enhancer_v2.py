...
#def get_relevant_context(user_question, limit=TOP_K): #ancienne version
...
def summarize_text(text, max_len=1000, min_len=30):
...
#def generate_prompt_paragraph(context, question, target_tokens=1000):
    ...
    for item in context:
        try:
            ...
            user_input = str(item[0])[:200]  # Truncate long questions
            ...
            if summary:
                ...
                conversation_summaries.append(entry)
    ...
    if keywords_used:
        ...
        if keywords_str:
            ...
            if current_length + len(kw_part.split()) <= target_tokens * 0.6:  # 60% du budget tokens
                parts.append(kw_part)
                current_length += len(kw_part.split())
    
    # 3. Résumé fluide en français
    if conversation_summaries:
        ...
        # Construction progressive pour respecter les tokens
        if len(summaries) == 1:
        ...
        if current_length + len(summary_text.split()) <= target_tokens - 30:  # Réserve pour la question
