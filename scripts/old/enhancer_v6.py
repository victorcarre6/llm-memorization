...
def generate_prompt_paragraph(context, question, target_tokens=1000):
    ...
    for item in context[:3]:  # Limiter à 3 éléments max
        try:
            ...
            # Nettoyage et segmentation du texte ===KBogus===
            #print(f"Avant segmentation : {len(llm_output.split())} mots")
            #cleaned_text = clean_and_segment_text(llm_output)

            # Summarization avec gestion de la longueur ===KBogus===
            print(f"Taille du texte à résumer : {len(llm_output.split())} mots")
            summary = summarize(
                text=llm_output,
                focus_terms=question_keywords,  # Utilisation directe des keywords
               #KBogus max_length=512
            )

           # summary = clean_summary(summary) ===KBogus===
